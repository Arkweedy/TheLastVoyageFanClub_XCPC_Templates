...
struct SegTree {
    ...
    inline void Push_up(int i) { M[i] = M[i << 1] * M[i << 1 | 1]; }
    ...
    Matrix Query_Tree(int left, int right, int i) {
        ...
        if (right <= mid)
            ...
            return Query_Tree(left, mid, i << 1) *
                   Query_Tree(mid + 1, right, i << 1 | 1);
    }
} T;
...
void update_path(int u, int w) {
    ...
    while (u != 0) {
        ...
        Val[u].mat[0][0] += max(aft.mat[0][0], aft.mat[1][0]) -
                            max(bef.mat[0][0], bef.mat[1][0]);
        Val[u].mat[0][1] = Val[u].mat[0][0];
